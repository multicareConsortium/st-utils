...
class NetworkMonitor:
    ...
    def report(self, interval: int = 30):
        ...
        with self._lock:
            ...
            logger.info(f"Periodic Health Report {"-"*(len(thread_line)-len("Period Health Report"))}")
            if not dead_threads: 
                logger.info(
                        thread_line
                        )
            ...
            if self.sensor_config_fail > 0:
                logger.warning(f"{self.sensor_config_fail} sensor configuration file/s are invalid!")
            ...
            for k, v in self.push_success.items():
                ...
                logger.info(f"Observations created for {k} → {v} (Time since last push: {time_since_last_push:.2f}m).")
            for k, v in self.push_fail.items():
                logger.warning(f"Rejected observations for {k} → {v}")
            for k, v in self.last_push_time.items():
                if v - time.time() > 3600:
                    logger.warning(f"Have not received observations from {k} in {v:.2f}.")
